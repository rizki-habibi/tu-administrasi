...
class KeuanganAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        $totalPemasukan = CatatanKeuangan::where('jenis', 'pemasukan')->where('status', 'approved')->sum('jumlah');
        $totalPengeluaran = CatatanKeuangan::where('jenis', 'pengeluaran')->where('status', 'approved')->sum('jumlah');
        $pendingCount = CatatanKeuangan::where('status', 'draft')->count();
    ...
    {
        $budgets = Anggaran::where('status', 'active')->get();
    ...
    public function show(CatatanKeuangan $keuangan)
    {
        $keuangan->load('creator', 'verifier');
        return view('admin.keuangan.show', compact('keuangan'));
    }

    public function verify(CatatanKeuangan $keuangan)
    {
        $keuangan->update([
            'status' => 'approved',
    ...
    public function destroy(CatatanKeuangan $keuangan)
    {
