...
class KesiswaanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $aktifCount = DataSiswa::where('status', 'aktif')->count();
        $lakiCount = DataSiswa::where('jenis_kelamin', 'L')->where('status', 'aktif')->count();
        $perempuanCount = DataSiswa::where('jenis_kelamin', 'P')->where('status', 'aktif')->count();
        $kelasList = DataSiswa::select('class')->distinct()->orderBy('class')->pluck('class');
    ...
    public function edit(DataSiswa $kesiswaan)
    {
        return view('admin.kesiswaan.edit', compact('kesiswaan'));
    }

    public function update(Request $request, DataSiswa $kesiswaan)
    {
        $request->validate([
    ...
    public function destroy(DataSiswa $kesiswaan)
    {
        if ($kesiswaan->foto) Storage::disk('public')->delete($kesiswaan->foto);
        $kesiswaan->delete();
