...
class DashboardController extends Controller
{
    ...
    {
        $user = auth()->user();
        $todayAttendance = Attendance::where('pengguna_id', $user->id)->whereDate('tanggal', today())->first();

        $monthlyStats = [
            'hadir' => Attendance::where('pengguna_id', $user->id)->where('status', 'hadir')->whereMonth('tanggal', now()->month)->count(),
            'terlambat' => Attendance::where('pengguna_id', $user->id)->where('status', 'terlambat')->whereMonth('tanggal', now()->month)->count(),
            'izin' => Attendance::where('pengguna_id', $user->id)->where('status', 'izin')->whereMonth('tanggal', now()->month)->count(),
            'sakit' => Attendance::where('pengguna_id', $user->id)->where('status', 'sakit')->whereMonth('tanggal', now()->month)->count(),
            'alpha' => Attendance::where('pengguna_id', $user->id)->where('status', 'alpha')->whereMonth('tanggal', now()->month)->count(),
        ];

        $pendingLeaves = LeaveRequest::where('pengguna_id', $user->id)->where('status', 'pending')->count();
        $upcomingEvents = Event::where('tanggal_acara', '>=', today())->where('status', 'upcoming')->orderBy('tanggal_acara')->take(5)->get();
        $unreadNotifications = Notification::where('pengguna_id', $user->id)->where('sudah_dibaca', false)->count();
        $recentNotifications = Notification::where('pengguna_id', $user->id)->latest()->take(5)->get();
