...
class DokumenWordController extends Controller
{
    public function index(Request $request)
    {
        ...
        $categories = DokumenWord::categories();
    ...
    {
        $templates = DokumenWord::templates();
        $categories = DokumenWord::categories();
    ...
    public function show(DokumenWord $word)
    {
    ...
    public function edit(DokumenWord $word)
    {
    ...
    public function update(Request $request, DokumenWord $word)
    {
        if ($word->pengguna_id !== auth()->id()) {
            abort(403);
        }
    ...
    public function destroy(DokumenWord $word)
    {
        if ($word->pengguna_id !== auth()->id()) {
            abort(403);
        }
    ...
    public function download(DokumenWord $word)
    {
        if ($word->pengguna_id !== auth()->id() && !$word->dibagikan) {
            abort(403);
        }
    ...
    public function aiGenerate(Request $request)
    {
        ...
        if ($request->document_id) {
            $doc = DokumenWord::where('id', $request->document_id)->where('pengguna_id', auth()->id())->first();
            if ($doc) {
    ...
    public function autosave(Request $request, DokumenWord $word)
    {
        if ($word->pengguna_id !== auth()->id()) {
            abort(403);
        }
