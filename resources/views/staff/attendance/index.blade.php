...
@php
    ...
    $monthStats = \App\Models\Attendance::where('user_id', auth()->id())
        ->whereMonth('date', now()->month)->whereYear('date', now()->year)
        ->selectRaw('status, COUNT(*) as total')->groupBy('status')->pluck('total','status');
    $officeLat = $setting->office_latitude ?? -8.1740;
    $officeLng = $setting->office_longitude ?? 113.7169;
    $maxDist   = $setting->max_distance_meters ?? 200;
@endphp
    ...
    <div class="card-body">
        @if(!$todayAttendance || !$todayAttendance->clock_in)
            <div class="text-center py-3">
                ...
                <p class="text-muted mb-1">Jam masuk: <strong>{{ $setting->clock_in_time ?? '08:00' }}</strong></p>
                @if($setting && $setting->late_tolerance_minutes)
                    <p class="text-muted" style="font-size:.82rem;">Toleransi: {{ $setting->late_tolerance_minutes }} menit</p>
                @endif
        ...
        @elseif(!$todayAttendance->clock_out)
            <div class="row g-3 align-items-center">
                <div class="col-md-6 text-center text-md-start">
                    ...
                    <p class="mb-2 text-muted">Jam Masuk: <strong class="text-dark">{{ $todayAttendance->clock_in }}</strong>
                ...
                @if($todayAttendance->latitude_in)
                <div class="col-md-6">
            ...
            <div class="row g-3 align-items-center">
                <div class="col-md-5 text-center text-md-start">
                    ...
                    <div class="d-flex gap-3 mt-2 justify-content-center justify-content-md-start flex-wrap">
                        <div><small class="text-muted d-block">Masuk</small><strong>{{ $todayAttendance->clock_in }}</strong></div>
                        <div><small class="text-muted d-block">Pulang</small><strong>{{ $todayAttendance->clock_out }}</strong></div>
                ...
                @if($todayAttendance->latitude_in || $todayAttendance->latitude_out)
                <div class="col-md-7">
...
<div class="modal fade" id="clockInModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius:16px;overflow:hidden;">
            ...
            <form id="clockInForm" action="{{ route('staff.attendance.clock-in') }}" method="POST">
                @csrf
...
<div class="modal fade" id="clockOutModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius:16px;overflow:hidden;">
            ...
            <form id="clockOutForm" action="{{ route('staff.attendance.clock-out') }}" method="POST">
                @csrf
        ...
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($att->latitude_in)
                            <a href="https://maps.google.com/?q={{ $att->latitude_in }},{{ $att->longitude_in }}" target="_blank" class="badge bg-light text-primary border text-decoration-none">
                    ...
                    <td>
                        <div class="d-flex gap-1 flex-wrap">
                            <a href="{{ route('staff.attendance.show', $att) }}" class="btn btn-sm btn-outline-primary" title="Detail"><i class="bi bi-eye"></i></a>
                            @if(!$att->date->isToday())
...
const HAS_OFFICE = {{ ($setting && $setting->office_latitude) ? 'true' : 'false' }};
...
function getLocation(type) {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
        ...
        if (addr) { addrEl.innerHTML = `<i class="bi bi-house-fill text-success me-1"></i>${addr}`; addrEl.classList.remove('d-none'); }
...
document.querySelectorAll('.captureBtn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        const photoI = document.getElementById('photo'+S);
        const wrap   = document.getElementById('cameraWrap'+S.charAt(0).toUpperCase()+S.slice(1));
...
document.querySelectorAll('.retakeBtn').forEach(btn => {
    btn.addEventListener('click', function() {
        const type=this.dataset.target, S=type==='in'?'In':'Out';
        document.getElementById('photo'+S).value='';
...
function checkReady(type) {
    const S=type==='in'?'In':'Out';
    const ok = document.getElementById('photo'+S)?.value && document.getElementById('lat'+S)?.value;
...
@if($todayAttendance && $todayAttendance->latitude_in && !$todayAttendance->clock_out)
document.addEventListener('DOMContentLoaded', () => {
    const m=L.map('miniMapIn',{zoomControl:false,attributionControl:false}).setView([{{ $todayAttendance->latitude_in }},{{ $todayAttendance->longitude_in }}],16);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(m);
    L.marker([{{ $todayAttendance->latitude_in }},{{ $todayAttendance->longitude_in }}],{icon:iconUser}).addTo(m).bindPopup('✅ Masuk {{ $todayAttendance->clock_in }}').openPopup();
    if(HAS_OFFICE) L.marker([OFFICE_LAT,OFFICE_LNG],{icon:iconOffice}).addTo(m).bindPopup('🏫 Sekolah');
});
@endif
@if($todayAttendance && $todayAttendance->clock_out && ($todayAttendance->latitude_in || $todayAttendance->latitude_out))
document.addEventListener('DOMContentLoaded', () => {
    const latC={{ $todayAttendance->latitude_in ?? $todayAttendance->latitude_out }};
    const lngC={{ $todayAttendance->longitude_in ?? $todayAttendance->longitude_out }};
    const m=L.map('miniMapBoth',{zoomControl:false,attributionControl:false}).setView([latC,lngC],15);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(m);
    @if($todayAttendance->latitude_in)
    L.marker([{{ $todayAttendance->latitude_in }},{{ $todayAttendance->longitude_in }}],{icon:iconUser}).addTo(m).bindPopup('✅ Masuk: {{ $todayAttendance->clock_in }}');
    @endif
    @if($todayAttendance->latitude_out)
    L.marker([{{ $todayAttendance->latitude_out }},{{ $todayAttendance->longitude_out }}],{icon:iconOutM}).addTo(m).bindPopup('🏠 Pulang: {{ $todayAttendance->clock_out }}');
    @endif
    if(HAS_OFFICE) L.marker([OFFICE_LAT,OFFICE_LNG],{icon:iconOffice}).addTo(m).bindPopup('🏫 Sekolah');
});
@endif
