            ...
            <div class="card-body">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1 overflow-hidden">
                        <h6 class="fw-medium mb-1" style="font-size:.85rem;">{{ Str::limit($doc->title, 35) }}</h6>
                        ...
                        <span class="badge bg-{{ $catColors[$doc->category] ?? 'secondary' }}" style="font-size:.65rem;">{{ ucfirst($doc->category) }}</span>
                    </div>
                </div>
                @if($doc->description)
                    <p class="text-muted mt-2 mb-0" style="font-size:.78rem;">{{ Str::limit($doc->description, 60) }}</p>
                @endif
            </div>
            <div class="card-footer bg-white border-0 pt-0 pb-3">
                <div class="d-flex justify-content-between align-items-center">
                    <small class="text-muted">{{ $doc->file_size_formatted }} &middot; {{ $doc->created_at->format('d/m/Y') }}</small>
                    <a href="{{ asset('storage/' . $doc->file_path) }}" class="btn btn-sm btn-outline-primary" target="_blank"><i class="bi bi-download"></i></a>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="uploadModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('staff.document.upload') }}" method="POST" enctype="multipart/form-data">
                @csrf
