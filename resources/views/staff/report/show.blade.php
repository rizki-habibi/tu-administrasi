...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div>
        @if($report->status == 'draft')
            <a href="{{ route('staff.report.edit', $report) }}" class="btn btn-warning"><i class="bi bi-pencil"></i> Edit</a>
            <form action="{{ route('staff.report.destroy', $report) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus laporan ini?')">
                @csrf @method('DELETE')
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <div class="row mb-3">
            ...
            <div class="col-md-9">{{ $report->title }}</div>
        </div>
        <div class="row mb-3">
            <div class="col-md-3 fw-bold">Kategori</div>
            <div class="col-md-9"><span class="badge bg-secondary">{{ ucfirst(str_replace('_',' ',$report->category)) }}</span></div>
        </div>
        <div class="row mb-3">
            <div class="col-md-3 fw-bold">Prioritas</div>
            <div class="col-md-9">
                @php $pc = ['rendah'=>'success','sedang'=>'warning','tinggi'=>'danger']; @endphp
                <span class="badge bg-{{ $pc[$report->priority] ?? 'secondary' }}">{{ ucfirst($report->priority) }}</span>
            </div>
        </div>
        <div class="row mb-3">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-9"><a href="{{ asset('storage/' . $report->attachment) }}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-download"></i> Download</a></div>
        </div>
        @endif
        <hr>
        <h6 class="fw-bold mb-3">Deskripsi</h6>
        <div class="bg-light p-3 rounded">{!! nl2br(e($report->description)) !!}</div>
    </div>
</div>
@endsection
