    ...
    <div class="row g-3">
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm border-start border-primary border-4">
                <div class="card-body">
                    <h6 class="mb-1">{{ $upcoming->judul }}</h6>
                    <p class="text-muted small mb-1">
                        <i class="bi bi-calendar3"></i> {{ $upcoming->tanggal_acara->format('d F Y') }}
                    </p>
                    <p class="text-muted small mb-1">
                        <i class="bi bi-clock"></i> {{ $upcoming->waktu_mulai }} - {{ $upcoming->waktu_selesai ?? 'Selesai' }}
                    </p>
                    @if($upcoming->lokasi)
                    <p class="text-muted small mb-0">
                        <i class="bi bi-geo-alt"></i> {{ $upcoming->lokasi }}
                    </p>
                    @endif
                </div>
            </div>
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                ...
                <span class="badge bg-{{ $typeColors[$event->jenis] ?? 'secondary' }} mb-2">{{ ucfirst($event->jenis) }}</span>
                <div class="text-muted small">
                    <p class="mb-1"><i class="bi bi-calendar3"></i> {{ $event->tanggal_acara->format('d F Y') }}</p>
                    <p class="mb-1"><i class="bi bi-clock"></i> {{ $event->waktu_mulai }} - {{ $event->waktu_selesai ?? 'Selesai' }}</p>
                    @if($event->lokasi)
                        <p class="mb-1"><i class="bi bi-geo-alt"></i> {{ $event->lokasi }}</p>
                    @endif
                </div>
                <p class="text-muted small mt-2 mb-0">{{ Str::limit($event->deskripsi, 100) }}</p>
            </div>
            <div class="card-footer bg-white border-0">
                <a href="{{ route('staf.agenda.show', $event) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i> Detail</a>
            </div>
        </div>
    </div>
    @empty
