...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="bi bi-pencil"></i> Ubah Data Buku</h4>
    <a href="{{ route('staf.buku.index') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>

<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form action="{{ route('staf.buku.update', $buku) }}" method="POST">
            @csrf @method('PUT')
            <div class="row g-3">
                <div class="col-md-8">
                ...
                <div class="col-md-3">
                    ...
                    <select name="kategori" class="form-select @error('kategori') is-invalid @enderror" required>
                        @foreach(\App\Models\BukuPerpustakaan::KATEGORI as $k => $v)
                            <option value="{{ $k }}" {{ old('kategori', $buku->kategori) == $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="lokasi_rak" class="form-control @error('lokasi_rak') is-invalid @enderror" value="{{ old('lokasi_rak', $buku->lokasi_rak) }}">
                    @error('lokasi_rak') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Jumlah Total <span class="text-danger">*</span></label>
                    <input type="number" name="jumlah_total" class="form-control @error('jumlah_total') is-invalid @enderror" value="{{ old('jumlah_total', $buku->jumlah_total) }}" min="1" required>
                    @error('jumlah_total') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold">Harga (Rp)</label>
                    <input type="number" name="harga" class="form-control @error('harga') is-invalid @enderror" value="{{ old('harga', $buku->harga) }}" min="0">
                ...
                <div class="col-md-4">
                    ...
                    <select name="sumber_dana" class="form-select @error('sumber_dana') is-invalid @enderror">
                        ...
                        @foreach(\App\Models\BukuPerpustakaan::SUMBER_DANA as $k => $v)
                            <option value="{{ $k }}" {{ old('sumber_dana', $buku->sumber_dana) == $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="kondisi" class="form-select @error('kondisi') is-invalid @enderror" required>
                        ...
                        <option value="rusak_ringan" {{ old('kondisi', $buku->kondisi) == 'rusak_ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                        <option value="rusak_berat" {{ old('kondisi', $buku->kondisi) == 'rusak_berat' ? 'selected' : '' }}>Rusak Berat</option>
                    </select>
                    @error('kondisi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-12">
                    <label class="form-label fw-bold">Keterangan</label>
                    <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror" rows="2">{{ old('keterangan', $buku->keterangan) }}</textarea>
