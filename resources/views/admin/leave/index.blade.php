        ...
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge bg-{{ $typeColors[$leave->type] ?? 'secondary' }}">{{ ucfirst(str_replace('_',' ',$leave->type)) }}</span></td>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('admin.leave.show', $leave) }}" class="btn btn-outline-info" title="Detail"><i class="bi bi-eye"></i></a>
                            @if($leave->status === 'pending')
                                <form action="{{ route('admin.leave.approve', $leave) }}" method="POST" class="d-inline" onsubmit="return confirm('Setujui pengajuan ini?')">
                                    @csrf @method('PATCH')
                                    <button class="btn btn-outline-success" title="Setujui"><i class="bi bi-check-lg"></i></button>
                                </form>
                                <form action="{{ route('admin.leave.reject', $leave) }}" method="POST" class="d-inline" onsubmit="return confirm('Tolak pengajuan ini?')">
                                    @csrf @method('PATCH')
