    ...
    <div class="card-body py-2">
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-4 d-flex gap-1">
                ...
                <a href="{{ route('admin.staff.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-counterclockwise"></i></a>
            </div>
        </form>
    </div>
</div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    <div class="fw-medium" style="font-size:.85rem;">{{ $staff->name }}</div>
                                    <small class="text-muted">{{ $staff->email }}</small>
                                </div>
                            </div>
                        </td>
                        <td style="font-size:.85rem;">{{ $staff->position ?? '-' }}</td>
                        <td style="font-size:.85rem;">{{ $staff->phone ?? '-' }}</td>
                        <td>
                            <span class="badge bg-{{ $staff->is_active ? 'success' : 'danger' }} bg-opacity-10 text-{{ $staff->is_active ? 'success' : 'danger' }} px-2 py-1" style="font-size:.75rem;">
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-1">
                                <button class="btn btn-light btn-sm" title="Lihat Detail"
                                        onclick="showDetail({{ $staff->id }}, '{{ addslashes($staff->name) }}', '{{ $staff->email }}', '{{ $staff->position ?? '-' }}', '{{ $staff->phone ?? '-' }}', '{{ $staff->address ?? '-' }}', '{{ $staff->is_active ? 'Aktif' : 'Nonaktif' }}', '{{ $staff->created_at->format('d M Y') }}')">
                                    <i class="bi bi-eye text-primary"></i>
                                </button>
                                <a href="{{ route('admin.staff.edit', $staff) }}" class="btn btn-light btn-sm" title="Edit"><i class="bi bi-pencil text-warning"></i></a>
                                <form method="POST" action="{{ route('admin.staff.toggle-status', $staff) }}" class="d-inline">
                                    @csrf @method('PATCH')
                                    <button class="btn btn-light btn-sm" title="{{ $staff->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                        <i class="bi bi-{{ $staff->is_active ? 'toggle-on text-success' : 'toggle-off text-secondary' }}"></i>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.staff.destroy', $staff) }}" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-light btn-sm" data-confirm="Hapus staff {{ $staff->name }}?" title="Hapus"><i class="bi bi-trash text-danger"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
