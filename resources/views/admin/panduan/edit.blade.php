...
@section('judul', 'Edit Panduan')

@section('konten')
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h4 class="fw-bold mb-1" style="color:var(--dark);">Edit Panduan</h4>
        <p class="text-muted mb-0" style="font-size:.82rem;">{{ $panduan->judul }}</p>
    </div>
    <a href="{{ route('admin.panduan.show', $panduan) }}" class="btn btn-outline-secondary btn-sm">
    ...
    <div class="row g-4">
            ...
            <div class="card" style="border:none;border-radius:var(--card-radius);box-shadow:0 1px 4px rgba(0,0,0,.06);">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" rows="2">{{ old('deskripsi', $panduan->deskripsi) }}</textarea>
                        @error('deskripsi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:.82rem;">Konten <span class="text-danger">*</span></label>
                        <textarea name="konten" class="form-control @error('konten') is-invalid @enderror" rows="18" style="font-family:monospace;font-size:.82rem;">{{ old('konten', $panduan->konten) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        @if($panduan->logo)
                            <div class="mb-2 d-flex align-items-center gap-3">
                                <img src="{{ $panduan->logo_url }}" alt="Logo" style="width:60px;height:60px;border-radius:10px;object-fit:cover;">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="hapus_logo" value="1" id="hapusLogo">
                                    <label class="form-check-label text-danger" for="hapusLogo" style="font-size:.78rem;">Hapus logo</label>
                                </div>
                            </div>
                        @endif
                        ...
                        <small class="text-muted">Maks 2MB. Kosongkan jika tidak ingin mengubah.</small>
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body pt-2">
                    <div class="mb-3">
                        ...
                        <select name="kategori" class="form-select form-select-sm @error('kategori') is-invalid @enderror" required>
                            @foreach(['panduan'=>'Panduan','dokumentasi'=>'Dokumentasi','changelog'=>'Changelog','referensi'=>'Referensi'] as $val => $label)
                                <option value="{{ $val }}" {{ old('kategori', $panduan->kategori) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="visibilitas" class="form-select form-select-sm @error('visibilitas') is-invalid @enderror" required>
                            <option value="semua" {{ old('visibilitas', $panduan->visibilitas) === 'semua' ? 'selected' : '' }}>Semua Pengguna</option>
                            <option value="admin" {{ old('visibilitas', $panduan->visibilitas) === 'admin' ? 'selected' : '' }}>Admin Only</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="ikon" class="form-control form-control-sm @error('ikon') is-invalid @enderror" value="{{ old('ikon', $panduan->ikon) }}">
                        @error('ikon') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        <small class="text-muted" style="font-size:.7rem;">Contoh: bi-book, bi-gear. <a href="https://icons.getbootstrap.com" target="_blank">Lihat ikon</a></small>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:.78rem;">Warna</label>
                        <input type="color" name="warna" class="form-control form-control-sm form-control-color" value="{{ old('warna', $panduan->warna ?? '#6366f1') }}" style="height:36px;">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:.78rem;">Versi</label>
                        <input type="text" name="versi" class="form-control form-control-sm" value="{{ old('versi', $panduan->versi) }}" placeholder="v1.0">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="font-size:.78rem;">Urutan</label>
                        <input type="number" name="urutan" class="form-control form-control-sm" value="{{ old('urutan', $panduan->urutan) }}" min="0">
                    </div>

                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" name="aktif" value="1" id="aktifSwitch" {{ old('aktif', $panduan->aktif) ? 'checked' : '' }}>
            ...
            <div class="d-grid gap-2">
                ...
                <a href="{{ route('admin.panduan.show', $panduan) }}" class="btn btn-outline-secondary">Batal</a>
            </div>
        </div>
    </div>
</form>
