...
<style>
    .markdown-body {
        ...
        font-size: .88rem; line-height: 1.7; max-width: 100%; padding: 28px 32px;
    }
    ...
    .doc-nav-list a:hover { background: #eef2ff; color: #6366f1; }
    .doc-nav-list a.current { background: #6366f1; color: #fff; font-weight: 600; }

    .btn-scroll-top { position: fixed; bottom: 30px; right: 30px; width: 44px; height: 44px; border-radius: 50%; background: #6366f1; color: #fff; border: none; box-shadow: 0 4px 14px rgba(99,102,241,.3); cursor: pointer; display: none; align-items: center; justify-content: center; font-size: 1.1rem; z-index: 1000; transition: all .2s; }
    .btn-scroll-top:hover { background: #4f46e5; transform: translateY(-2px); }
    .btn-scroll-top.show { display: flex; }

    .meta-info { font-size: .75rem; color: #94a3b8; display: flex; gap: 14px; flex-wrap: wrap; margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,.15); }
    ...
    .action-dropdown .dropdown-item:hover { background: #eef2ff; color: #6366f1; }
    ...
    @media print {
        ...
        .print-cover .cover-border {
            position: absolute; inset: 8mm; border: 3px solid #312e81; border-radius: 6px;
        }
        .print-cover .cover-border::before {
            content: ''; position: absolute; inset: 4px; border: 1px solid #6366f1; border-radius: 4px;
        }
        .print-cover .cover-ornament { width: 100px; height: 3px; background: linear-gradient(90deg, transparent, #6366f1, transparent); margin: 0 auto; }
        .print-cover .cover-institution { font-size: 10pt; color: #64748b; text-transform: uppercase; letter-spacing: 3px; margin-top: 10px; }
        .print-cover .cover-school { font-size: 13pt; font-weight: 700; color: #312e81; margin-bottom: 30px; }
        .print-cover .cover-divider { width: 70px; height: 3px; background: #6366f1; margin: 0 auto 24px; border-radius: 2px; }
        ...
        .print-cover .cover-version { display: inline-block; background: #eef2ff; color: #4338ca; padding: 4px 16px; border-radius: 16px; font-size: 9pt; font-weight: 600; margin-bottom: 40px; }
        .print-cover .cover-meta-table { border-collapse: collapse; font-size: 9pt; }
        ...
        .markdown-body h2 { font-size: 12pt; color: #312e81; }
        ...
        .print-footer {
            display: block !important; text-align: center; font-size: 8pt;
            color: #94a3b8; margin-top: 30px; padding-top: 10px;
...
<div class="print-cover">
    ...
    <div style="width:60px;height:60px;border-radius:14px;background:linear-gradient(135deg,#312e81,#6366f1);display:flex;align-items:center;justify-content:center;margin-bottom:16px;">
    ...
    <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
        <div class="d-flex align-items-start">
            ...
            <div>
                ...
                <div class="meta-info">
                    <span><i class="bi bi-person"></i>{{ $panduan->pembuat->nama ?? '-' }}</span>
                    ...
                    @if($panduan->visibilitas === 'admin')<span><i class="bi bi-lock-fill"></i>Admin Only</span>@endif
                </div>
            </div>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('admin.panduan.index') }}" class="header-btn">
                <i class="bi bi-arrow-left"></i> Kembali
            </a>
            <a href="{{ route('admin.panduan.edit', $panduan) }}" class="header-btn">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    <li>
                        <button class="dropdown-item" onclick="window.print()">
                            <i class="bi bi-printer text-primary"></i> Cetak dengan Sampul
                        </button>
                    </li>
                    <li>
                        <button class="dropdown-item" id="btnDownloadPdf">
                            <i class="bi bi-file-earmark-pdf text-danger"></i> Download PDF
                        </button>
                    </li>
                    <li>
                        <a class="dropdown-item" href="{{ route('admin.panduan.download', $panduan) }}">
                            <i class="bi bi-filetype-html text-warning"></i> Download HTML
                        </a>
                    </li>
                    <li><hr class="dropdown-divider"></li>
                    <li>
                        <form action="{{ route('admin.panduan.upload-drive', $panduan) }}" method="POST" id="formGDrive">
                            @csrf
                            <button class="dropdown-item" type="submit" id="btnGDrive">
                                <i class="bi bi-google text-success"></i> Simpan ke Google Drive
                            </button>
                        </form>
                    </li>
                </ul>
...
<div class="row">
    ...
    <div class="col-lg-3 d-none d-lg-block">
        ...
        <div class="card">
            <div class="card-body p-3">
                <h6 class="fw-bold mb-2" style="font-size:.78rem; color:#475569;">
                    <i class="bi bi-folder2-open me-1"></i> Dokumen Lainnya
                </h6>
                <div class="doc-nav-list">
                    @foreach($dokumenList as $d)
                        <a href="{{ route('admin.panduan.show', $d) }}" class="{{ $d->id === $panduan->id ? 'current' : '' }}">
...
<button class="btn-scroll-top" id="btnScrollTop" title="Kembali ke atas">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnDownloadPdf').addEventListener('click', function() {
        ...
        if (coverBorder) {
            ...
            coverBorder.style.border = '3px solid #312e81';
