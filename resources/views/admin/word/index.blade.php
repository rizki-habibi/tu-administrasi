...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('admin.word.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>Buat Dokumen Baru</a>
    </div>
</div>
    ...
    <div class="col-6 col-lg-3">
        <div class="card border-0 shadow-sm">
            <div class="card-body text-center py-3">
                ...
                <h4 class="fw-bold mb-0 mt-1">{{ App\Models\WordDocument::where('is_shared', true)->count() }}</h4>
    ...
    <div class="card-body py-3">
        <form method="GET" class="row g-2 align-items-end">
            ...
            @if(request()->hasAny(['search','category','status']))
                <div class="col-md-2">
                    <a href="{{ route('admin.word.index') }}" class="btn btn-sm btn-outline-secondary w-100"><i class="bi bi-x-circle me-1"></i>Reset</a>
                </div>
            @endif
        </form>
    </div>
        ...
        <div class="row g-2">
            @foreach(App\Models\WordDocument::templates() as $key => $tpl)
                ...
                <div class="col-6 col-md-4 col-lg-3">
                    <a href="{{ route('admin.word.create', ['template' => $key]) }}" class="card border h-100 text-decoration-none hover-shadow" style="transition:box-shadow .2s;">
                        <div class="card-body py-3 px-3">
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <i class="bi bi-file-earmark-text text-primary"></i>
                                <small class="fw-bold text-dark">{{ $tpl['name'] }}</small>
                            </div>
                            <small class="text-muted" style="font-size:.72rem;">{{ $tpl['description'] }}</small>
                        </div>
                    </a>
                </div>
                @endif
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.word.show', $doc) }}" class="fw-semibold text-decoration-none">
                                <i class="bi bi-file-earmark-word text-primary me-1"></i>{{ Str::limit($doc->title, 40) }}
                            </a>
                        </td>
                        <td><span class="badge bg-primary-subtle text-primary">{{ $categories[$doc->category] ?? $doc->category }}</span></td>
                        <td><small>{{ $doc->user->name ?? '-' }}</small></td>
                        <td>
                            @if($doc->status == 'draft')
                        ...
                        <td class="text-center">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('admin.word.show', $doc) }}" class="btn btn-outline-primary" title="Lihat"><i class="bi bi-eye"></i></a>
                                <a href="{{ route('admin.word.edit', $doc) }}" class="btn btn-outline-success" title="Edit"><i class="bi bi-pencil"></i></a>
                                <a href="{{ route('admin.word.download', $doc) }}" class="btn btn-outline-info" title="Download .docx"><i class="bi bi-download"></i></a>
                                <form action="{{ route('admin.word.destroy', $doc) }}" method="POST" class="d-inline" onsubmit="return false;">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            ...
                            <a href="{{ route('admin.word.create') }}" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg me-1"></i>Buat Dokumen</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
