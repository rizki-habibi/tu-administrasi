<!DOCTYPE html>
<html><head>
    <meta charset="utf-8"><title>Data Kehadiran - TU Administrasi</title>
    <style>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr><td>{{ $i+1 }}</td><td>{{ $a->user->name ?? '-' }}</td><td>{{ $a->date->format('d/m/Y') }}</td><td>{{ $a->clock_in ?? '-' }}</td><td>{{ $a->clock_out ?? '-' }}</td><td>{{ ucfirst($a->status) }}</td><td>{{ $a->note ?? '-' }}</td></tr>
            @endforeach
        </tbody>
    </table>
