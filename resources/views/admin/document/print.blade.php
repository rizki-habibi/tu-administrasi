...
<html><head>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $i + 1 }}</td><td>{{ $doc->title }}</td><td>{{ ucfirst($doc->category) }}</td>
                <td>{{ $doc->file_name }}</td><td>{{ $doc->file_size_formatted }}</td>
                <td>{{ $doc->uploader->name ?? '-' }}</td><td>{{ $doc->created_at->format('d/m/Y') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
