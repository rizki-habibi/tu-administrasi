...
@section('judul', 'Detail Event')

@section('konten')
<div class="mb-3">
    <a href="{{ route('admin.agenda.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <h6 class="mb-0"><i class="bi bi-calendar-event"></i> {{ $event->judul }}</h6>
                <div class="d-flex gap-2">
                    <span class="badge bg-{{ $typeColors[$event->jenis] ?? 'secondary' }}">{{ ucfirst($event->jenis) }}</span>
            ...
            <div class="card-footer bg-white d-flex gap-2">
                <a href="{{ route('admin.agenda.edit', $event) }}" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i> Edit</a>
                <form action="{{ route('admin.agenda.destroy', $event) }}" method="POST" onsubmit="return confirm('Yakin hapus event ini?')">
                    @csrf @method('DELETE')
